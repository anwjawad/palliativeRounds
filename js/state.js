/* ------------------------------------------------------
 ...
 (keeps all existing behaviors + fixes)
-------------------------------------------------------*/
(function () {
  ...
  const off = (evt, fn) => (listeners[evt] = (listeners[evt] || []).filter((h) => h !== fn));
  const emit = (evt, payload) => (listeners[evt] || []).forEach((h) => h(payload));

  /* =============== Schema Helpers =============== */
  // Blank ESAS (1–10; null means not selected)
  const blankESAS = () => ESAS_FIELDS.reduce((acc, k) => ((acc[k] = null), acc), {});
  ...
  const blankLabs = () => {
    const make = (keys) => keys.reduce((acc, k) => ((acc[k] = null), acc), {});
    return {
  ...
  const newPatient = (partial = {}) => ({
    id: uid("pt"),
    section: (partial.section || "A"), // سيتم تعديله لاحقًا لاحترام currentSection
    done: false,
  ...
  const ensureSchema = (p) => {
    ...
    if (!p.ctcae) p.ctcae = blankCTCAE();
      ...
      Object.keys(base.items).forEach((k) => {
        ...
        base.items[k].grade = cur && CTCAE_GRADES.includes(Number(cur.grade))
          ? Number(cur.grade)
          : (cur?.grade === 0 ? 0 : null);
      });
    ...
    // مهم لعدم “تسريب” النصوص بين المرضى:
  ...
  const addPatient = (partial) => {
    // احترام القسم الحالي إذا لم يُمرَّر في partial
    const section = state.ui.currentSection || "A";
    const base = newPatient({ ...partial, section: (partial && partial.section) ? partial.section : section });
    const p = ensureSchema(base);
    // ضمان التوقيت
    p.updatedAt = fmtTime(new Date());
  ...
  const updatePatient = (id, patch) => {
    ...
    if (state.ui.currentPatientId === id) emit("current:changed", getCurrentPatient());
    return true;
  };
  ...
  const getLabValue = (patient, key) => {
    ...
    for (const g of ["group1", "group2", "group3"]) {
      if (key in labs[g]) {
        return labs[g][key] == null || labs[g][key] === "" ? LAB_DEFAULT : String(labs[g][key]);
      }
    }
  ...
  const importRows = (rows) => {
    ...
    const createdIds = [];
    const sect = state.ui.currentSection || "A";
    rows.forEach((row) => {
      const bio = {};
      HOSPITAL_HEADERS.forEach((h) => (bio[h] = row[h] ?? ""));
      const id = addPatient({ bio, section: sect }); // احترام القسم الحالي وقت الاستيراد
      createdIds.push(id);
    });
  ...
  const exportRows = () => {
    // Export exactly hospital headers (for round-trip)
  ...
  PR.state = {
    ...
    on, off, emit,
    // patients
    addPatient, updatePatient, removePatient, setSection,
    setCurrentPatient, getCurrentPatient, markDone, progress, searchPatients,
    // labs
    getLabValue,
    // csv
    ...
    restore, persist,
    // schema utils
    blankESAS, blankCTCAE, blankLabs, blankHPI, blankBio, newPatient, ensureSchema,
  };
})();